...
def initialize_values(graph, source):
    residue_values = {(source, u): 0 if (source, u) != (source, source) else 1 for u in graph.nodes}
    reserve_values = {(source, u): 0 for u in graph.nodes}
    PPR_values = {(source, v): 0 for v in graph.nodes}
    random_walks_numbers = {(source, u): 0 for u in graph.nodes}
...
def random_walks(random_walks_numbers, source, alpha, residue_values, k_s, current_node):
    random_walks_numbers[current_node] = (1 - alpha) * residue_values[(source, current_node)] * k_s
    return random_walks_numbers

def forward_push(graph, source, alpha, residue_values, reserve_values, PPR_values, random_walks_numbers, k_s):
