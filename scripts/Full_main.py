...
ALPHA = 0.5
EPSILON = 0.5
FILE_PATH = '0.8_test.txt'
OUT_PUT_FILE_WEIGHT = '0.8_weight.txt'
OUT_PUT_FILE_TIMESTAMP = '0.8_timestamp.txt'
OUT_PUT_FILE_ANOMALY_SCORES = "0.8_anomalyscore.txt"
OUT_PUT_FILE_MAX_ANOMALY_SCORE = "0.8_Max_anomalyscore.txt"
OUT_PUT_FILE_WT_ANOMALY_NODES = "0.8_WT_anomaly_nodes.txt"
OUT_PUT_FILE_ANOMALY_NODES = "0.8_anomalous_nodes.txt"
OUT_PUT_FILE_FINAL_RESULTS = "0.8test_final_results.txt"
...
# Add code snippet to merge nodes and write to "final_results.txt" file
# Read nodes from "WT_anomaly_nodes.txt" file
with open(OUT_PUT_FILE_WT_ANOMALY_NODES, 'r') as file:
    wt_anomaly_nodes = [line.strip() for line in file]

# Read nodes from "WDDS_test_output_results.txt" file
with open("WDDS_test_output_results.txt", 'r') as file:
    wdds_nodes = [line.strip() for line in file]

# Merge the lists of nodes
final_nodes = set(wt_anomaly_nodes + wdds_nodes)

# Write merged nodes to "final_results.txt" file
with open(OUT_PUT_FILE_FINAL_RESULTS, 'w') as file:
    file.write("Merged Nodes:\n")
...
print(f"Merged nodes have been saved to {OUT_PUT_FILE_FINAL_RESULTS}")
